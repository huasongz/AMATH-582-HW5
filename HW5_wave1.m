...
v = VideoReader('wave1.mp4');
nFrames = v.NumberOfFrames;
IMG = [];
for i=1:nFrames
img = rgb2gray(read(v,i)); % get one RGB image
img = reshape(img,720*1280,1);
IMG = [IMG img];
end
IMG = double(IMG);
...
xlabel('Image Framse')
title('Singular Value Spectrum on wave video')
print(gcf,'-dpng','rank_wave.png');
r=1; 
...
for i = 1:nFrames
    subplot(1,3,1)
    img1 = uint8(IMG(:,i));
    imshow(reshape(img1,720,1280))
    title('Original Video')
    subplot(1,3,2)
    img2 = uint8(Xdmd(:,i));
    imshow(reshape(img2,720,1280))
    title('Background')
    subplot(1,3,3)
    img3 = real(X_sparse_dmd(:,i));
    imshow(reshape(img3,720,1280))
    title('Foreground')
    drawnow
end
print(gcf,'-dpng','Images_wave.png');

%%
for i = 1:nFrames
    img1 = uint8(X_lr_dmd(:,i));
    imshow(reshape(img1,720,1280))
    title('Background with R matrix added')
end
